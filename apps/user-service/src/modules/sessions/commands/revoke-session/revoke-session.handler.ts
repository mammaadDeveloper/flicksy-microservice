...
export class RevokeSessionCommandHandler implements ICommandHandler<RevokeSessionCommand>{
    constructor(
        @InjectDataSource()
    ...
    async execute(command: RevokeSessionCommand): Promise<SessionEntity> {
        ...
        try{
            const sessionEntity = await queryRunner.manager.findOne(SessionEntity, {where: {jti}});
